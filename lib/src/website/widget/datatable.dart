...
/// - see [DataTable3.smRatio], [DataTable3.lmRatio] and same properties on [PaginatedDataTable3].
...
class DataColumn2 extends DataColumn {
  /// Creates the configuration for a column of a [DataTable3].
  ///
...
class DataRow2 extends DataRow {
  ...
  /// Creates the configuration for a row of a [DataTable3].
  ///
...
class DataTable3 extends DataTable {
  DataTable3({
    super.key,
  ...
  /// Note: to change background color of fixed data rows use [DataTable3.headingRowColor]
  ...
  Widget build(BuildContext context) {
    ...
    if (dataTableShowLogs && kDebugMode) {
      debugPrint('DataTable3 built: ${sw.elapsedMilliseconds}ms');
    }
  ...
  List<double> _calculateDataColumnSizes(BoxConstraints constraints,
    ...
    assert(totalFixedWidth < totalColAvailableWidth,
        "DataTable3, combined width of columns of fixed width is greater than availble parent width. Table will be clipped");
